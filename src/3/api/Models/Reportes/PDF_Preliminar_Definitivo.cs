...
namespace api.Models.Reportes
{
  ...
  public class Deptos
  {
    ...
    /// </summary>
    public List<Diagnosticos_Pre> Diags { get; set; } = new List<Diagnosticos_Pre>();
  }

  /// <summary>
  /// Genera el documento preliminar y definitivo
  /// </summary>
  public class PDF_Preliminar_Definitivo
  {
    #region [Propiedades]
    ...
    /// </summary>
    public PDFFlowable_Table tabla_mpios = new PDFFlowable_Table();

    private Paths paths;
    private PDFFlowable pdf;
    #endregion
    ...
    public PDF_Preliminar_Definitivo(string _tipo_, Reporte _reporte_, Eventos _evento_, List<Diagnosticos_Pre> _diags_)
    {
      reporte = _reporte_;
      tipo = _tipo_;
      evento = _evento_;
    ...
    public string Create()
    {
      ...
      string save_path = $"{paths.Fisical_Path}\\reportespdf\\{nuevo_nombre}.pdf";

      // >>>>> Crear el pdf flowable
      pdf = new PDFFlowable(save_path);
      // >>>>> Agregar paragraphs
      // datos del json
      List<string> textos;
      textos = paths.Data.Data_Parse_List_String("textos");
      /*** Titulos ***/
      Set_Titulos();

      List<Deptos> _diags_;
      _diags_ = Get_Departamentos(diags);
      /*** Primer Texto ***/
      ...
      pdf.Add = header;
      /*** Pié de página ***/
      PDFFlowable_Header footer = new PDFFlowable_Header(false)
      {
        Textos = textos.ElementAt(1).Data_Parse("texto").Split('|').ToList(),
        WithSeparator = (true, "#123367")
      };
      pdf.Add = footer;
      pdf.LineBreak();
      ...
      return $"{paths.Url_Path}/ReportesPDF/{nuevo_nombre}.pdf";
    }
    #endregion
    ...
    private void Set_Tabla_Municipios(List<string> textos, List<Deptos> _mpios_)
    {
      ...
      foreach (Deptos d in _mpios_)
      {
        ...
        total_area += area;
        double familias = Get_PobAfectada(diags);
        total_familia += familias;
        double total = Get_CostoTotal(diags);
        total_total += total;
    ...
    private void Set_Texto_2(List<string> textos, List<Deptos> _diags_)
    {
      ...
      {
        pdf.Style_Apply = "Normal_Left";
        pdf.Add = titulo_deptos.Replace("__depto__", d.Keys);
    ...
    private void Set_Titulos()
    {
      ...
      pdf.Style_Apply = "Heading1";
      pdf.Add = titulo1.ToUpper(); //titulo1 del json
      pdf.Style_Apply = "Heading2";
      pdf.Add = titulo2.ToUpper();
      pdf.Style_Apply = "Heading3";
      pdf.Add = titulo3;
    }
    #endregion
